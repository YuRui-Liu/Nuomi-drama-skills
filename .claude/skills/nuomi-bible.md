...
每个圣经文件必须同时包含两个部分，缺一不可：

1. **Markdown 表格/正文** — 人类可读的详细描述（创作期使用）
2. **末尾 JSON 块（` ```json ... ``` `）** — 机器可读的结构化数据，用于 `load_bible()` → `emit_registry()` → Dashboard 资源库 → 锚图生成管线

JSON 块格式模板：
- 角色: `{"characters": {"canonical_id": {"外貌": "具体出图描述", "aliases": ["别名1"], "性格": "", "动机": ""}}}`
- 场景: `{"scenes": {"场景名": {"appearance": "空间/光线/材质描述", "氛围": ""}}}`
- 道具: `{"props": {"道具名": {"appearance": "材质/颜色/形状", "关联角色": "角色id"}}}`

**没有 JSON 块 → registry.json 为空 → Dashboard 资源库为空 → 出图管线无法生成锚图。**
